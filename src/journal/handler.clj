(ns journal.handler
  (:require [compojure.core :refer [defroutes]]
            [journal.routes.home :refer [home-routes open-routes]]
            [journal.middleware :refer [load-middleware]]
            [journal.session-manager :as session-manager]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "journal.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  ;;start the expired session cleanup job
  (cronj/start! session-manager/cleanup-job)
  (timbre/info "\n-=[ journal started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "journal is shutting down...")
